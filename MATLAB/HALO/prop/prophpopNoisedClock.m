...
function dXdt = prophpopNoisedClock(t,X,model)
    ...
    % Allen deviation due to unmodeled source (from the table of clock noise)
    % ---------------------------------------------------------------------------------------------------------- %
    if isfield(model, 'noise') && ~isempty(model.noise)
        dt_noise = model.noise.dt;
        clc_table = model.noise.clc;

        % Find the current interval index
        idx = floor((t - model.seq.Time) / dt_noise);
        if idx < size(clc_table, 1)
            clc_rand = clc_table(idx+1);
        else
            % If it exceeds the last interval, it will be treated as the last interval unchanged.
            clc_rand = clc_table(end);
        end
    else
        clc_rand = 0;
    end
    ...
    % accleration in total
    % ---------------------------------------------------------------------------------------------------------- %
    acc_total = acc_centralPlanet + acc_earth + acc_pointMasses + acc_SRPSun + acc_alb + acc_genRel;
    % ---------------------------------------------------------------------------------------------------------- %

    % assembly
    % ---------------------------------------------------------------------------------------------------------- %
    dXdt = zeros(8,1);
    dXdt(1:3) = vJ2000;    % Position differential = velocity
    dXdt(4:6) = acc_total; % Velocity differential = acceleration
    dXdt(7)   = drift;     % b' = db
    dXdt(8)   = clc_rand;    
    % ---------------------------------------------------------------------------------------------------------- %
end
